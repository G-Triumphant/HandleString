...
#import "NWDStringUtil.h"
#import <RegexKitLite/RegexKitLite.h>

@implementation NWDStringUtil
...
{
    return [NSString stringWithFormat:@"%@%%", [self interceptionDecimalString:[NSNumber numberWithDouble:doubleValue] digitsNumber:digits]];
}
...
//格式化截取小数点后N位 digitsNumber代表N
+ (NSString *)interceptionDecimalString:(NSObject *)object digitsNumber:(NSInteger)digitsNumber
{
    if (object) {
        NSMutableString *money = [NSMutableString stringWithString:object.description];
        NSRange dotRangeD = [money rangeOfString:@"."];
        if (NSNotFound == dotRangeD.location) {
            [money appendString:@"."];
        }
        ...
        if (NSNotFound != dotRange.location) {
            ...
            while (cnt++ < digitsNumber) {
                [money appendString:@"0"];
            }
            decimal = [money substringWithRange:NSMakeRange(dotRange.location, digitsNumber+1)];
...
{
    NSString *formatMoney = [NWDStringUtil formatMoney:object];
...
{
//    return [NWDStringUtil validateByRegex:@"^[\u4e00-\u9fa5]"];
...
//        isSuccess = [NWDStringUtil validateByRegex:@"^((13[0-9])|(14[5|7])|(15[0|1|2|3|5|6|7|8|9])|(17[6|7|8])|18[0-9])\\d{8}|(170[0|5|9]\\d{7})$"];
...
{
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:[NWD_SAFE_STRING(rate) stringByAppendingString:NWD_SAFE_STRING(addRate)]];
